...
* dla każdego kroku $$(\Delta_i)$$ istnieje przyporządkowany stan $$(s_i)$$ (lub jego reprezentacja w logu / kodzie) tak, że można porównać
  ...
  $$E(\Delta_i)\quad\text{z}\quadL\big(H(s_i), a^*(s_i)\big)$$
...
Definiujemy **energię epizodu** jako
...
$$
\widehat{R}_{\text{ep}}(F,g)
:= \frac{1}{\lvert I_{\text{ep}} \rvert} \sum_{i \in I_{\text{ep}}} E_i
$$
...
Funkcje `estimate_episode_risk` i `estimate_mosaic_risk` są **dokładną implementacją** odpowiednio estymatorów
\widehat{R}_{\mathrm{episode}}(F,g)
\quad\text{oraz}\quad
\widehat{R}_{\mathrm{mosaic}}(F,g)
dla struktur danych `MosaicStep` emitowanych przez `analyze_module_source`.
...
$$
   ...
   $$\big\lvert E(\Delta_i) - L\big(H(s_i), a^*(s_i)\big) \big\rvert \le C$$
...
$$
\widehat{R}_{\text{gate}}^{\text{ep}}(F,g)
:=
\frac{1}{\lvert I_{\text{gate}} \rvert}
\sum_{i \in I_{\text{gate}}} E_i
$$
